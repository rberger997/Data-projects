...
nba.all <- read.csv('~/Desktop/My\ files/nbaallelo.csv')
nba.all[1:10,]
...
x90s[1:10, ]
mj.opp <- nba.all[nba.all$is_playoffs == 1 & nba.all$fran_id == 'Bulls' & nba.all$year_id > 1985 & nba.all$year_id <= 1998,]
...
lj.cle <- nba.all[nba.all$is_playoffs == 1 & nba.all$fran_id == 'Cavaliers' & nba.all$year_id > 2003 & nba.all$year_id < 2010,]
...
colnames(lj.oppavg) <- c('Team', 'Year', 'ELO')
